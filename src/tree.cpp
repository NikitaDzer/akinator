...
//   memset(p_tree, 0x0, sizeof(Tree) * 1 + sizeof(TreeNode) * initialCapacity);
   ...
   p_tree->final->yeapSon = nodes;
   p_tree->final->nopeSon = nullptr;
...
tree_destruct(Tree *const p_tree, bitmask_t *const p_bitmask)
{
   ...
   if (p_tree->final->nopeSon != nullptr)
   {
      TreeNode *subfinal = p_tree->final->nopeSon;
      TreeNode *temp     = nullptr;
      
      while (true)
      {
         if (subfinal->nopeSon == nullptr)
         {
            free(subfinal->yeapSon);
            break;
         }
         
         temp = subfinal->nopeSon;
         free(subfinal->yeapSon);
         subfinal = temp;
      }
   }
...
tree_add_node(Tree *const p_tree, const TreeNodeData *const p_data, bitmask_t *const p_bitmask)
{
   ...
   if (p_tree->free == p_tree->final)
   {
      ...
      final->yeapSon = section;
      final->nopeSon = p_tree->final;
...
TreeNode*
tree_relate(TreeNode *const p_daddy, TreeNode *const p_son, TreeNode **const p_relation, bitmask_t *const p_bitmask)
{
#ifdef    TREE_ANTI_FOOL
   if (p_daddy == nullptr || p_son == nullptr)
   {
